...
videoContainer.addEventListener('wheel', function(event) { /* ... */ }, { passive: false });
videoContainer.addEventListener('mousedown', function(event) { /* ... */ });
videoContainer.addEventListener('mouseleave', function(event) { /* ... */ });
videoContainer.addEventListener('click', function(event) { /* ... */ }, true);

// ========= 関数定義 =========
// (applyZoomPan, resetZoomPan, updateDebugOverlay, updateDataTable, drawScalePoint, clearScaleOverlay は変更なし)
function applyZoomPan() { /* ... */ }
function resetZoomPan() { /* ... */ }
function updateDebugOverlay() { /* ... */ }
function updateDataTable() { /* ... */ }
function drawScalePoint(videoPoint) { /* ... */ }
function clearScaleOverlay() { /* ... */ }

/**
 * スケール設定時にSVGに線を描画するヘルパー関数
 * ★★★ この関数の中身を修正します ★★★
 */
function drawScaleLine(videoP1, videoP2) {
    const svgP1_x = videoP1.x * scale + translateX;
    const svgP1_y = videoP1.y * scale + translateY;
    const svgP2_x = videoP2.x * scale + translateX;
    const svgP2_y = videoP2.y * scale + translateY;

    // リアルタイム描画のために、もし既存の線があれば一度削除する
    const existingLine = scaleOverlay.querySelector('line');
    if (existingLine) { existingLine.remove(); }

    const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    line.setAttribute('x1', svgP1_x);
    line.setAttribute('y1', svgP1_y);
    line.setAttribute('x2', svgP2_x);
    line.setAttribute('y2', svgP2_y);
    
    // ▼▼▼ 抜け落ちていたスタイルの指定を3行追加 ▼▼▼
    line.setAttribute('stroke', 'yellow');
    line.setAttribute('stroke-width', 2);
    line.setAttribute('stroke-dasharray', '5 5'); // 破線を指定
    
    scaleOverlay.appendChild(line);
}
...
videoContainer.addEventListener('mouseup', function(event) {isDragging = false;videoContainer.classList.remove('dragging');if (hasDragged || !videoPlayer.paused) {return;}if (isScalingMode) {const rect = videoContainer.getBoundingClientRect();const containerX = event.clientX - rect.left;const containerY = event.clientY - rect.top;const point = { x: (containerX - translateX) / scale, y: (containerY - translateY) / scale };scalePoints.push(point);drawScalePoint(point);if (scalePoints.length === 1) {alert("始点を設定しました。次に「終点」をクリックしてください。");} else if (scalePoints.length === 2) {drawScaleLine(scalePoints[0], scalePoints[1]);const realDistanceStr = prompt("今クリックした2点間の実際の距離を「メートル(m)」単位で入力してください。", "1.0");if (realDistanceStr !== null && !isNaN(realDistanceStr) && Number(realDistanceStr) > 0) {const realDistance = Number(realDistanceStr);const p1 = scalePoints[0];const p2 = scalePoints[1];const pixelDistance = Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));scaleRatio = pixelDistance / realDistance;scaleDisplay.textContent = `スケール: ${scaleRatio.toFixed(2)} px/m`;alert(`スケールを設定しました: ${scaleRatio.toFixed(2)} px/m`);updateDataTable();} else {alert("無効な値です。スケール設定をキャンセルしました。");}isScalingMode = false;scalePoints = [];setTimeout(clearScaleOverlay, 100);}} else if (isUpdateMode) {const rect = videoContainer.getBoundingClientRect();const containerX = event.clientX - rect.left;const containerY = event.clientY - rect.top;const newX = (containerX - translateX) / scale;const newY = (containerY - translateY) / scale;trackingData[updateIndex].x = newX;trackingData[updateIndex].y = newY;isUpdateMode = false;updateIndex = null;updateDataTable();const message = "データを更新しました。\n\n[OK] を押すと、最新の計測時間に戻ります。\n[キャンセル] を押すと、この時間から続けて計測します。";if (confirm(message)) {if (trackingData.length > 0) {const lastTime = trackingData[trackingData.length - 1].t;videoPlayer.currentTime = lastTime;}}} else {const rect = videoContainer.getBoundingClientRect();const containerX = event.clientX - rect.left;const containerY = event.clientY - rect.top;const originalX = (containerX - translateX) / scale;const originalY = (containerY - translateY) / scale;if (originalX < 0 || originalY < 0 || originalX > videoPlayer.videoWidth || originalY > videoPlayer.videoHeight) { return; }const time = videoPlayer.currentTime;if (dataMode === 'overwrite') {const existingIndex = trackingData.findIndex(p => p.t === time);if (existingIndex !== -1) {trackingData[existingIndex].x = originalX;trackingData[existingIndex].y = originalY;} else {trackingData.push({ t: time, x: originalX, y: originalY });}} else {trackingData.push({ t: time, x: originalX, y: originalY });}trackingData.sort((a, b) => a.t - b.t);updateDataTable();const framesToAdvance = parseInt(intervalInput.value, 10) || 1;videoPlayer.currentTime += framesToAdvance / FRAME_RATE;}});
